unit uConFornecedor;

interface

uses
  Winapi.Windows, Winapi.Messages, System.SysUtils, System.Variants, System.Classes, Vcl.Graphics,
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, uTelaHerancaConsulta, Data.DB, FireDAC.Stan.Intf, FireDAC.Stan.Option,
  FireDAC.Stan.Param, FireDAC.Stan.Error, FireDAC.DatS, FireDAC.Phys.Intf, FireDAC.DApt.Intf, FireDAC.Stan.Async,
  FireDAC.DApt, FireDAC.Comp.DataSet, FireDAC.Comp.Client, Vcl.Grids, Vcl.DBGrids, Vcl.StdCtrls, Vcl.Buttons, Vcl.Mask,
  Vcl.ExtCtrls;

type
  TfrmConFornecedor = class(TfrmTelaHerancaConsulta)
    f1QryListagemfornId: TFDAutoIncField;
    f2QryListagemnome: TStringField;
    f2QryListagemcnpj: TStringField;
    f2QryListagemendereco: TStringField;
    f2QryListagemtelefone: TStringField;
    f2QryListagememail: TStringField;
    f2QryListagemobservacao: TStringField;
    procedure FormCreate(Sender: TObject);
  private
    { Private declarations }
  public
    { Public declarations }
  end;

var
  frmConFornecedor: TfrmConFornecedor;

implementation

{$R *.dfm}

procedure TfrmConFornecedor.FormCreate(Sender: TObject);
begin
  inherited;
  aCampoId :='fornId';
  IndiceAtual :='nome';
end;

end.
